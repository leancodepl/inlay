...
void _writeStoreClass(StringBuffer buffer, StoreDefinition store) {
  ...
  // Constructor.
  buffer.write('  $className(this._storage');
  ...
  // Storage field.
  buffer.writeln('  final KeyValueStorage _storage;');
  ...
  buffer.writeln('  // ── Getters (async, always fresh) ─────────────────────────────');
  buffer.writeln();
  ...
  buffer.writeln('  // ── Reactive Stream ───────────────────────────────────────────');
  buffer.writeln();

  final snapshotClassName = '${className}Snapshot';
  buffer.writeln('  Stream<$snapshotClassName> get stream {');
  buffer.writeln('    return _storage.stream');
  buffer.writeln("        .where((entries) => entries.any((e) => e.key.startsWith(_key(''))))");
  buffer.writeln('        .asyncMap((_) => getSnapshot());');
  buffer.writeln('  }');
  buffer.writeln();

  // getSnapshot() method.
  buffer.writeln('  Future<$snapshotClassName> getSnapshot() async {');
  buffer.writeln('    return $snapshotClassName(');
  for (final field in valueFields) {
  ...
  buffer.writeln();
  buffer.writeln('  Future<void> clear() async {');
  buffer.writeln("    await _storage.removeByPrefix(_key(''));");
  buffer.writeln('  }');
...
void _writeSnapshotClass(StringBuffer buffer, StoreDefinition store) {
  ...
  buffer.writeln('/// Immutable snapshot of ${store.className} values.');
  buffer.writeln('class $className {');
...
void _writeGetter(StringBuffer buffer, FieldInfo field) {
  ...
  buffer.writeln('  Future<$returnType> $getterName() async {');
  buffer.writeln("    final value = await _storage.getString(_key('${field.name}'));");
